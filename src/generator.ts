...
async function createFiles(
  ...
  const spriteJson: SpriteJsonType = {};
  let x = imagePadding;
  let y = imagePadding;
  let initWidth = Math.ceil(
    Math.sqrt(
      sortedImages.reduce((a, b) => a + (b.width + imagePadding) * (b.height + imagePadding), 0)
    )
  );
  const maxWidth =
    Math.ceil(sortedImages.reduce((a, b) => (a.width > b.width ? a : b)).width) + imagePadding;
  ...
  const file = await sharp({
    ...
    .toBuffer();
  const bin = {
    w: height < initWidth ? initWidth : height,
    h: height < initWidth ? initWidth : height,
  };
  ...
  await sharp(file, {
    ...
    .png()
    .toFile(resolve(join(outputPath, `${outputFileName}.png`)));
